...
function computeOneTeamAch(tn,tr,tc,mt,firstEpoch){
  ...
  (function(){var rivals=[];for(var otn in rankings){if(otn===tn)continue;var tro=rankings[otn];var wins=0,losses=0,total=0;tc.forEach(function(e){var orn=getRank(tro[e.compId]);if(orn===null||orn===undefined||orn==='')return;var rn=parseInt(e.rank,10);orn=parseInt(orn,10);if(isNaN(rn)||isNaN(orn))return;if(rn>20||orn>20)return;if(rn<orn)wins++;else if(orn<rn)losses++;});if(wins+losses>=8&&Math.abs(wins-losses)<=1)rivals.push('对'+otn+' '+wins+':'+losses);}if(rivals.length>0)ach.push({id:'rival',tier:'silver',name:'宿敌',detail:rivals.join('\n')});})();
